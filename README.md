...
**Головний** створює ціль, додає виконавців (Telegram user id, одноразовий код або посилання `https://t.me/<бот>?start=КОД`), бачить усю картину: прогрес, етапи, хто на чому, чергу на перевірку. Закриває або відхиляє етап.
...
5. `OWNER_TELEGRAM_ID` — Telegram id головного. Цей акаунт отримує роль owner під час першого входу. Інші, хто відкриває Mini App, — виконавці (вхід лише через Telegram `initData`, без email).
6. Запрошення: головний копіює `https://t.me/<BOT_USERNAME>?start=КОД`. `/start КОД` відкриває Mini App з `?invite=КОД`; після перевірки initData застосунок сам гасить код і відкриває ціль.

## Змінні (.env.example)

- `BOT_TOKEN` — токен бота
- `BOT_USERNAME` — нік бота без @ (для посилання `t.me/<нік>?start=КОД`)
